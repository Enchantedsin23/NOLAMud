...
/*
 * Retrieve character's current appearance.
 */
...
void affect_modify (CHAR_DATA * ch, AFFECT_DATA * paf, bool fAdd)
{
	...
	case APPLY_CHIMERSTRY:
		if (!IS_NPC (ch))
			ch->pcdata->powers_mod[DISC_CHIMERSTRY] += mod;
		break;
	case APPLY_DAIMOINON:
		if (!IS_NPC (ch))
			ch->pcdata->powers_mod[DISC_DAIMOINON] += mod;
		break;
	case APPLY_DOMINATE:
		if (!IS_NPC (ch))
	...
	case APPLY_FORTITUDE:
		if (!IS_NPC (ch))
			ch->pcdata->powers_mod[DISC_FORTITUDE] += mod;
		break;
	case APPLY_DEMENTATION:
		if (!IS_NPC (ch))
			ch->pcdata->powers_mod[DISC_DEMENTATION] += mod;
		break;
	case APPLY_NECROMANCY:
		if (!IS_NPC (ch))
			ch->pcdata->powers_mod[DISC_NECROMANCY] += mod;
		break;
	case APPLY_OBEAH:
		if (!IS_NPC (ch))
			ch->pcdata->powers_mod[DISC_OBEAH] += mod;
		break;
	...
	case APPLY_OBTENEBRATION:
		if (!IS_NPC (ch))
			ch->pcdata->powers_mod[DISC_OBTENEBRATION] += mod;
		break;
	case APPLY_POTENCE:
		if (!IS_NPC (ch))
			ch->pcdata->powers_mod[DISC_POTENCE] += mod;
		break;
	case APPLY_PRESENCE:
		if (!IS_NPC (ch))
			ch->pcdata->powers_mod[DISC_PRESENCE] += mod;
		break;
	case APPLY_PROTEAN:
		if (!IS_NPC (ch))
	...
	case APPLY_QUIETUS:
		if (!IS_NPC (ch))
			ch->pcdata->powers_mod[DISC_QUIETUS] += mod;
		break;
	case APPLY_SERPENTIS:
		if (!IS_NPC (ch))
			ch->pcdata->powers_mod[DISC_SERPENTIS] += mod;
		break;
	case APPLY_MORTIS:
		if (!IS_NPC (ch))
			ch->pcdata->powers_mod[DISC_MORTIS] += mod;
		break;
	case APPLY_THAUMATURGY:
		if (!IS_NPC (ch))
			ch->pcdata->powers_mod[DISC_THAUMATURGY] += mod;
		break;
	case APPLY_VICISSITUDE:
		if (!IS_NPC (ch))
			ch->pcdata->powers_mod[DISC_VICISSITUDE] += mod;
		break;
	...
	case APPLY_SET_FORTITUDE:
		...
		ch->pcdata->powers_set_wear++;
		break;
	case APPLY_SET_DEMENTATION:
		if (!IS_NPC (ch) && ch->pcdata->powers_set[DISC_DEMENTATION] < mod)
			ch->pcdata->powers_set[DISC_DEMENTATION] = mod;
		ch->pcdata->powers_set_wear++;
		break;
	...
	case APPLY_SET_SERPENTIS:
		...
		ch->pcdata->powers_set_wear++;
		break;
	case APPLY_SET_MORTIS:
		if (!IS_NPC (ch) && ch->pcdata->powers_set[DISC_MORTIS] < mod)
			ch->pcdata->powers_set[DISC_MORTIS] = mod;
		ch->pcdata->powers_set_wear++;
		break;
...
void char_from_room (CHAR_DATA * ch)
{
	...
	if (!IS_NPC (ch) && (IS_WEREWOLF (ch)) && IS_SET (ch->act, PLR_VANISH))
	{
		REMOVE_BIT (ch->act, PLR_VANISH);
		send_to_char ("Your movement has removed possiblity of camouflage.\n\r", ch);
	}
...
bool can_see (CHAR_DATA * ch, CHAR_DATA * victim)
{
	...
	if (IS_VAMPAFF (victim, VAM_OBFUSCATE) && !IS_VAMPIRE (ch) && !IS_WEREWOLF (ch) && !IS_STORYTELLER (ch))
		return FALSE;
	...
	if (IS_VAMPAFF (victim, VAM_OBFUSCATE))
	{
		...
		else if (IS_VAMPAFF (victim, VAM_OBFUSCATE) && IS_VAMPIRE (victim) && IS_VAMPIRE (ch) && get_disc (ch, DISC_AUSPEX) < get_disc (victim, DISC_OBFUSCATE) && get_disc (ch, DISC_CHIMERSTRY) < get_disc (victim, DISC_OBFUSCATE))
			return FALSE;
...
char *affect_loc_name (int location)
{
	...
	case APPLY_GENERATION:
		return "Modifies generation by";
	case APPLY_ANIMALISM:
		return "Modifies Animalism discipline by";
	case APPLY_AUSPEX:
		return "Modifies Auspex discipline by";
	case APPLY_CELERITY:
		return "Modifies Celerity discipline by";
	case APPLY_CHIMERSTRY:
		return "Modifies Chimerstry discipline by";
	case APPLY_DAIMOINON:
		return "Modifies Daimoinon discipline by";
	case APPLY_DOMINATE:
		return "Modifies Dominate discipline by";
	case APPLY_FORTITUDE:
		return "Modifies Fortitude discipline by";
	case APPLY_DEMENTATION:
		return "Modifies Dementation discipline by";
	case APPLY_NECROMANCY:
		return "Modifies Necromancy discipline by";
	case APPLY_OBEAH:
		return "Modifies Obeah discipline by";
	case APPLY_OBFUSCATE:
		return "Modifies Obfuscate discipline by";
	case APPLY_OBTENEBRATION:
		return "Modifies Obtenebration discipline by";
	case APPLY_POTENCE:
		return "Modifies Potence discipline by";
	case APPLY_PRESENCE:
		return "Modifies Presence discipline by";
	case APPLY_PROTEAN:
		return "Modifies Protean discipline by";
	case APPLY_QUIETUS:
		return "Modifies Quietus discipline by";
	case APPLY_SERPENTIS:
		return "Modifies Serpentis discipline by";
	case APPLY_MORTIS:
		return "Modifies Mortis discipline by";
	case APPLY_THAUMATURGY:
		return "Modifies Thaumaturgy discipline by";
	case APPLY_VICISSITUDE:
		return "Modifies Vicissitude discipline by";
	case APPLY_BLOOD_MAX:
		return "Modifies blood pool by";
	case APPLY_BLOOD_POT:
		return "Modifies blood potency by";
	case APPLY_SET_ANIMALISM:
		return "Gives min Animalism discipline of";
	case APPLY_SET_AUSPEX:
		return "Gives min Auspex discipline of";
	case APPLY_SET_CELERITY:
		return "Gives min Celerity discipline of";
	case APPLY_SET_CHIMERSTRY:
		return "Gives min Chimerstry discipline of";
	case APPLY_SET_DAIMOINON:
		return "Gives min Daimoinon discipline of";
	case APPLY_SET_DOMINATE:
		return "Gives min Dominate discipline of";
	case APPLY_SET_FORTITUDE:
		return "Gives min Fortitude discipline of";
	case APPLY_SET_DEMENTATION:
		return "Gives min Dementation discipline of";
	case APPLY_SET_NECROMANCY:
		return "Gives min Necromancy discipline of";
	case APPLY_SET_OBEAH:
		return "Gives min Obeah discipline of";
	case APPLY_SET_OBFUSCATE:
		return "Gives min Obfuscate discipline of level";
	case APPLY_SET_OBTENEBRATION:
		return "Gives min Obtenebration discipline of";
	case APPLY_SET_POTENCE:
		return "Gives min Potence discipline of";
	case APPLY_SET_PRESENCE:
		return "Gives min Presence discipline of";
	case APPLY_SET_PROTEAN:
		return "Gives min Protean discipline of";
	case APPLY_SET_QUIETUS:
		return "Gives min Quietus discipline of";
	case APPLY_SET_SERPENTIS:
		return "Gives min Serpentis discipline of";
	case APPLY_SET_MORTIS:
		return "Gives min Mortis discipline of";
	case APPLY_SET_THAUMATURGY:
		return "Gives min Thaumaturgy discipline of";
	case APPLY_SET_VICISSITUDE:
		return "Gives min Vicissitude discipline of";
	}
