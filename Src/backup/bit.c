...
char *room_bit_name (int room_flags)
{
	...
	if (room_flags & ROOM_UNDER_WATER)
		strcat (buf, " under_water");
	if (room_flags & ROOM_SMALLCAERN)
		strcat (buf, " smallcaern");
	if (room_flags & ROOM_LARGECAERN)
		strcat (buf, " largecaern");
	if (room_flags & ROOM_DEFILEDCAERN)
		strcat (buf, " defiledcaern");
return (buf[0] != '\0') ? buf + 1 : "";
}
...
int room_name_bit (char *buf)
{
	...
	if (!str_cmp (buf, "under_water"))
		return ROOM_UNDER_WATER;
	if (!str_cmp (buf, "total_darkness"))
		return ROOM_TOTAL_DARKNESS;
	if (!str_cmp (buf, "smallcaern"))
		return ROOM_SMALLCAERN;
	if (!str_cmp (buf, "largecaern"))
		return ROOM_LARGECAERN;
	if (!str_cmp (buf, "defiledcaern"))
		return ROOM_DEFILEDCAERN;
	return 0;
