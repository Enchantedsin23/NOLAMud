...
/*	,
	{"Immortals", "Immortal Discussion", LEVEL_STORYTELLER, LEVEL_STORYTELLER, "imm", DEF_NORMAL, 60, NULL, FALSE}
	,
	{"Group", "Communications from your group", 0, 1, "all", DEF_NORMAL, 28, NULL, FALSE}
	,
	{"Roleplay", "Whispers in the Dark", 0, 1, "all", DEF_NORMAL, 30, NULL, FALSE }
*/
};
...
static void show_note_to_char (CHAR_DATA * ch, NOTE_DATA * note, int num)
{
	...
	/* Ugly colors ? */
	sprintf (buf, "[" BOLD "%4d" NO_COLOR "] " BOLD YELLOW "%s" NO_COLOR ": " GREEN "%s" NO_COLOR "\n\r" BOLD YELLOW "Board" NO_COLOR ": %s\n\r" BOLD YELLOW "Date" NO_COLOR ":  %s\n\r" BOLD YELLOW "To" NO_COLOR ":    %s\n\r" GREEN "===========================================================================" NO_COLOR "\n\r" "%s\n\r", num, note->sender, note->subject, note->board->short_name, note->date, note->to_list, note->text);
...
void do_board (CHAR_DATA * ch, char *argument)
{
	...
	if (!argument[0])	/* show boards */
	{
		...
		count = 1;
		send_to_char (BOLD BLUE "Board system now Online.\n\r" 
		BLUE "http://twilightmu.org/forum" NO_COLOR "\n\r", ch);
		for (i = 0; i < MAX_BOARD; i++)
		{
			unread = unread_notes (ch, &boards[i]);	/* how many unread notes? */
			if (unread != BOARD_NOACCESS)
			{
				sprintf (buf, BOLD "%2d" NO_COLOR "> " CYAN BOLD "%12s" NO_COLOR " [%s%4d" NO_COLOR "] " BLUE "%s" NO_COLOR "\n\r", count, boards[i].short_name, unread ? RED : GREEN, unread, boards[i].long_name);
				send_to_char (buf, ch);
				count++;
			}	/* if has access */
