...
//				}
	...
	if (!str_cmp (arg1, "brawl"))
	{
		...
		pMob->abilities[TALENTS][TAL_BRAWL] = atoi (arg2);
	...
	if (!str_cmp (arg1, "dodge"))
	{
		if (arg2[0] == '\0' || !is_number (arg2))
		{
			send_to_char ("Syntax: dodge [number]\n\r", ch);
			return;
		}

		pMob->abilities[TALENTS][TAL_DODGE] = atoi (arg2);

		SET_BIT (pArea->area_flags, AREA_CHANGED);
		send_to_char ("Dodge set.\n\r", ch);
		return;
	}

	if (!str_cmp (arg1, "melee"))
	{
		if (arg2[0] == '\0' || !is_number (arg2))
		{
			send_to_char ("Syntax:  melee [number]\n\r", ch);
			return;
		}

		pMob->abilities[SKILLS][SKI_MELEE] = atoi (arg2);

		SET_BIT (pArea->area_flags, AREA_CHANGED);
		send_to_char ("Melee set.\n\r", ch);
		return;
	}
...
void do_mshow (CHAR_DATA * ch, char *argument)
{
	...
	sprintf (buf, "\nBrawl:   %d", victim->abilities[TALENTS][TAL_BRAWL]);
	send_to_char (buf, ch);
	sprintf (buf, "\tDodge:   %d", victim->abilities[TALENTS][TAL_DODGE]);
	send_to_char (buf, ch);
	sprintf (buf, "\tMelee:   %d\n\r", victim->abilities[SKILLS][SKI_MELEE]);
	send_to_char (buf, ch);
	...
	for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
	{
		for (pMobIndex = mob_index_hash[iHash]; pMobIndex != NULL; pMobIndex = pMobIndex->next)
		{
			if (pMobIndex != NULL && pMobIndex->area == pArea)
			{
				...
				fprintf (fp, "%d ", pMobIndex->abilities[TALENTS][TAL_BRAWL]);
				fprintf (fp, "%d ", pMobIndex->abilities[TALENTS][TAL_DODGE]);
				fprintf (fp, "%d \n", pMobIndex->abilities[SKILLS][SKI_MELEE]);
	...
	for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
	{
		for (pObjIndex = obj_index_hash[iHash]; pObjIndex != NULL; pObjIndex = pObjIndex->next)
		{
			if (pObjIndex != NULL && pObjIndex->area == pArea)
			{
				...
				{
					fprintf (fp, "Q\n%s~\n%s~\n%s~\n%s~\n%s~\n%s~\n%d %d\n", pObjIndex->chpoweron, pObjIndex->chpoweroff, pObjIndex->chpoweruse, pObjIndex->victpoweron, pObjIndex->victpoweroff, pObjIndex->victpoweruse, pObjIndex->spectype, pObjIndex->specpower);
				}
			}
		}
	}
		...
		for (pRoomIndex = room_index_hash[iHash]; pRoomIndex; pRoomIndex = pRoomIndex->next)
		{
			if (pRoomIndex->area == pArea)
			{
				...
				for (door = 0; door < MAX_DIR; door++)
				{
					...
					{
						int locks = 0;
						if (IS_SET (pExit->rs_flags, EX_ISDOOR) && (!IS_SET (pExit->rs_flags, EX_PICKPROOF)) && (!IS_SET (pExit->rs_flags, EX_BASHPROOF)) && (!IS_SET (pExit->rs_flags, EX_PASSPROOF)))
							locks = 1;
						if (IS_SET (pExit->rs_flags, EX_ISDOOR) && (IS_SET (pExit->rs_flags, EX_PICKPROOF)) && (!IS_SET (pExit->rs_flags, EX_BASHPROOF)) && (!IS_SET (pExit->rs_flags, EX_PASSPROOF)))
							locks = 2;
						if (IS_SET (pExit->rs_flags, EX_ISDOOR) && (!IS_SET (pExit->rs_flags, EX_PICKPROOF)) && (IS_SET (pExit->rs_flags, EX_BASHPROOF)) && (!IS_SET (pExit->rs_flags, EX_PASSPROOF)))
...
void do_arealist (CHAR_DATA * ch, char *argument)
{
	...
	for (pArea = area_first; pArea != NULL; pArea = pArea->next)
	{
		sprintf (addbuf, "#b[#B%3d#b]#n #C%-35s#n #e(#w%-5d-%5d#e)#n\n\r      #r%13s#n #e%2d#n #b[#w%s#b]#n\n\r", 
