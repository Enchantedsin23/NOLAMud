...
{
	send_to_char ("#c.-----------#n Entering#w L#nong#wD#nesc#n Editor#c -----------.\n\r", ch);
	send_to_char ("#c|#y •#C Type #g.#Gf#C on a new line to format. #c            |\n\r", ch);
	send_to_char ("#c|#y •#C Type #g.#Gp#C to check for any misspelled words.#c   |\n\r", ch);
  send_to_char ("#c|#y •#C Terminate with a #G@#C on a blank line.        #c  |\n\r", ch);
	send_to_char ("#c'------------------------------------------------'#n\n\r", ch);
...
void string_add (CHAR_DATA * ch, char *argument)
{
	...
	if (*argument == '.')
	{
		...
		send_to_char( "#CAccording to #GDictionary#g.#Gcom#C, you have #Rmisspelled#C the following#c:#n\n\r",ch);
		...
		if (!str_cmp (arg1, ".c"))
		{
			send_to_char ("#wS#ntring#R cleared#n.\n\r", ch);
			**ch->desc->pString = '\0';
			return;
		}
		...
		if (!str_cmp (arg1, ".f"))
		{
			*ch->desc->pString = format_string (*ch->desc->pString);
			send_to_char ("#CYour#w L#nong#wD#nesc#C has been formatted#c.#n\n\r", ch);
			return;
		}
