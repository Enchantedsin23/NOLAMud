...
#if defined(macintosh) || defined(MSDOS)
   boot_db ();
   log_string ("Innocence Lost is up", 
CHANNEL_LOG_NORMAL);
   game_loop_mac_msdos ();
#endif
   ...
   boot_db (fCopyOver);
   sprintf (log_buf, "Innocence Lost is running on port %d.", port);
...
#endif
         {
            ...
            if (!read_from_descriptor (d))
            {
               if (d->character != NULL)
				...
				// Dream code
				{
				  if (d->character->inDream != NULL)
				  do_sever(d->character, "");
                  do_autosave (d->character, "");
				}
				// end dream
               d->outtop = 0;
         ...
         if (FD_ISSET (d->descriptor, &in_set))
         {
            ...
            if (!read_from_descriptor (d))
            {
			   if (d->character != NULL && d->character->onPhone != NULL)
				   do_hangup(d->character, "");
			   // Dream code
			   	if (d->character != NULL && d->character->inDream != NULL)
				do_sever(d->character, "");
			   // End Dream
               FD_CLR (d->descriptor, &out_set);
...
#define FNDELAY O_NDELAY
   ...
   { 
      ...
      sprintf (log_buf, "Socket IP:  %s", buf);
   ...
   for (pban = ban_list; pban != NULL; pban = pban->next)
   {
      ...
      {
         write_to_descriptor (desc, "Your site is banned from Innocence Lost.\n\r", 0);
         close (desc);
...
void read_from_buffer (DESCRIPTOR_DATA * d)
{
    ...
    */
	  ...
	  //telnet negotiation (Cone)
	   ...
	   {
			sprintf(buf, "Illegal character received: %d", (unsigned int)(*((unsigned char *)&d->inbuf[i])));
...
bool process_output (DESCRIPTOR_DATA * d, bool fPrompt)
{
   ...
   {
      write_to_buffer (d,
      "#R[ Press (c)ontinue, (r)efresh, (b)ack, (q)uit or return ]#n", 0);
   }
   else if (fPrompt && !merc_down && d->connected == CON_PLAYING)
   {
      ...
      else if (IS_SET (ch->act, PLR_PROMPT) && ch->desc->editor != ED_REDITOR)
      {
         ...
         char claritymin[MAX_INPUT_LENGTH];
         char claritymax[MAX_INPUT_LENGTH];
	...
	if (IS_EXTRA (ch, EXTRA_OSWITCH))
	{
         ...
         else
         {
            ...
            COL_SCALE (exp_str, ch, ch->pcdata->willpower[WILLPOWER_MAX], 100);
               ...
               sprintf (claritymin, "%d", ch->pcdata->clarity[0]);
            COL_SCALE (exp_str, ch, ch->pcdata->clarity[0], 100);
               sprintf (claritymax, "%d", ch->pcdata->clarity[1]);
            COL_SCALE (exp_str, ch, ch->pcdata->clarity[1], 100);
	      ...
	      /* Lumi's Code */
	      sprintf (convcurrent, "%d", ch->pcdata->harmony[HARMONY_CURRENT]);
	    COL_SCALE (exp_str, ch, ch->pcdata->harmony[HARMONY_CURRENT], 100);
            /* done */
	    if (!IS_NPC(ch))
            {
           ...
           if (IS_STORYTELLER(ch)){
           		if (IS_AFFECTED(ch, AFF_POLYMORPH))	
           			sprintf (buf, "\n\r#G<#evnum#G:#e%d#G> #G<#eDisguise#G:#e %s#G> ", ch->in_room->vnum, ch->morph);
           		else
           			sprintf (buf, "\n\r#G<#evnum#G:#e%d#G> ", ch->in_room->vnum);
		    ...
		    sprintf (buf, "#y[#gWillpower %s#y/#g%s#y] [#gBlood %s#y/#g%s#y] <#g%s#y>#n",wpmin,wpmax,bloodmax,bloodmin,spokenword);    
		        if (IS_VAMPAFF (ch, VAM_FANGS))
                  		strcat(buf, "#n <#RF#n>");
                  	if (IS_MORE2 (ch, MORE2_HEIGHTENSENSES))
                  		strcat(buf, "#n <#CHS#n>");
                  	if ((IS_MORE3(ch, MORE3_OBFUS3)) || (IS_MORE3(ch, MORE3_OBFUS1)))
                  		strcat(buf, "#e [Hidden]#n");
                  	if (IS_VAMPAFF (ch, VAM_DISGUISED))
                  		strcat(buf, "#e [Hidden]#n");
                  	if (IS_MORE2 (ch, MORE2_RPFLAG))
                  		strcat(buf, "#g <#GIC#g>#n");
                    /* Vehicle prompt */
                  	if ( ch->vehicle != NULL )
					            strcat(buf, "#n <#wDriving#n>");
					          /* End vehicle prompt */

					if (ch->onPhone != NULL)
					{
						if (ch->phonestate == 1)
							strcat(buf, "#y <#gPhone#y>#n");
						else if (ch->phonestate == 0)
							sprintf( buf, "%s #y<#gIncoming Call From:#w %s#y>#n", buf, ch->onPhone->name);
					...
					{
						if (ch->dreamstate == 1)
							strcat(buf, "#C <#cDreaming#C>#n");
						else if (ch->phonestate == 0)
							sprintf( buf, "%s #C<#cSleepy#C>#n", buf);
						else if (ch->phonestate == -1)
							strcat(buf, "#C <#cDreaming#C>#n");
					}
					// End Dream
                     ...
                     sprintf (buf, "#e[#cWillpower %s#e/#c%s#e]#e [#cGlamour %s#e/#c%s#e] <#c%s#e>#n",wpmin,wpmax,bloodmax, bloodmin, spokenword);
                     	if (IS_AFFECTED (ch, AFF_SHADOWPLANE))
                     		strcat(buf, "#c <#BUmbra#c>");
						
						if ( ch->pcdata->condition[COND_DRUNK] > 10 )
	                    strcat(buf, "#y <Drunk>#n");
					
						if ( ch->pcdata->condition[COND_DRUNK] > 2 && ch->pcdata->condition[COND_DRUNK] < 11)
                        strcat(buf, "#y <Tipsy>#n");
					
                        if (ch->sex == SEX_FEMALE && IS_EXTRA(ch, EXTRA_LABOUR))
                        strcat(buf, "#R <In Labour>#n");
                     
					    if (((get_eq_char (ch, WEAR_LEGS)) == NULL) && ((get_eq_char (ch, WEAR_BODY)) == NULL) && ((get_eq_char (ch, WEAR_FEET)) == NULL) && ((get_eq_char (ch, WEAR_ABOUT)) == NULL))
						strcat(buf, "#R <#rNaked#R>#n");
					 ...
					 if (ch->onPhone != NULL)
					{
						if (ch->phonestate == 1)
							...
							sprintf( buf, "%s #y<#gIncoming Call From#y:#w %s#y>#n", buf, ch->onPhone->name);
						else if (ch->phonestate == -1)
							strcat(buf, "#y <#gPhone#y>#n");
					}
										// Dream Code
					...
					// End Dream
					   ...
					   sprintf (buf, "#g<#GWP %s#g/#G%s#g> <#P%s#p>#n", wpmin, wpmax, spokenword);
                       if (IS_MORE2 (ch, MORE2_RPFLAG))
						   strcat(buf, "#g <#GIC#g>");
						           if ( ch->vehicle != NULL )
					     strcat(buf, "#o <#yVehicle#o>");
					...
					// End Dream
					   ...
					   sprintf (buf, "#e<#cWP %s/%s#e> <#cPlasm %d#e> <#c%s#e>#n", wpmin, wpmax, ch->pcdata->plasm, spokenword);
                       if (IS_MORE2 (ch, MORE2_RPFLAG))
						   strcat(buf, "#g <#GIC#g>");
						           if ( ch->vehicle != NULL )
					     strcat(buf, "#o <#yVehicle#o>");
					   ...
					   if (ch->onPhone != NULL)
						{
							if (ch->phonestate == 1)
								strcat(buf, "#C <#nPhone#C>#n");
							else if (ch->phonestate == 0)
								sprintf( buf, "%s #C<#nIncoming Call From:#w %s#C>#n", buf, ch->onPhone->name);
							else if (ch->phonestate == -1)
								strcat(buf, "#C <#nPhone#C>#n");
						}
											// Dream Code
					...
					// End Dream
                     ...
                     sprintf (buf, "#e[#cWillpower %s#e/#c%s#e] <#c%s#e>#n",wpmin,wpmax,spokenword);
                     if (IS_MORE2 (ch, MORE2_RPFLAG))
                  		strcat(buf, "#g<#GIC#g>#n");
                  	 if ( ch->vehicle != NULL )
					            strcat(buf, "#n <#wDriving#n>");
...
/* Delete leading spaces UNLESS character is writing a note */
   ...
   case CON_GET_NAME:
      ...
      {
         /* Old player */
         write_to_buffer (d, "#ePlease enter your password#o:#n ", 0);
      ...
      else
      {
         ...
         sprintf (buf, "#eAre you certain #w%s#e is an appropriate name for modern Los Angeles? #w(#eY#w/#eN#w)#e?#n ", argument);
         write_to_buffer (d, buf, 0);
   ...
   case CON_GET_OLD_PASSWORD:
    ...
    ProtocolNoEcho( d, false );
	  ...
	  log_string (log_buf, CHANNEL_LOG_CODER);
        ...
        {
             write_to_buffer(d,"#yPlease enter your site username (lishmud.com). If you don't have one yet, no worries! Just enter 'None.' Site username : #n ",0);
      ...
      for( eban = eban_list; eban; eban = eban->next )
      {
         ...
         {
           send_to_char("That site username has been banned.\n\r", ch);
           close_socket (d);
           return;
         }
      }
      ...
      /*if (ch->level >= 7)
      {
         stc ("#eLog on in`G w`yi`Gz`y-`Gi`yn`Gv`yi`Gs`e mode?#e (`wy`e/`wn`e):`n` ", ch);
...
//      do_help (ch, "motd");
	 ...
	 // do_help (ch, "changes");
     do_function( d->character, &do_changes, "" );
   ...
   case CON_GET_EMAIL:
   {
      ...
      if( argument[0] == '\0' )
      {
         write_to_buffer( d, "You MUST enter something, be it 'None' or your site username.\n\r", 0);
         write_to_buffer( d, "Site username: ", 0 );
         d->connected = CON_GET_EMAIL;
       	 break; 
      }   
      ...
      if (ch->level >= 7)
      {
         stc ("#eLog on in `Gw`yi`Gz`y-`Gi`yn`Gv`yi`Gs`y m`Go`yd`Ge`e?#e (`wy`e/`wn`e)`e:`n` ", ch);
         d->connected = CON_WIZ_INVIS;
         return;
      }
//      do_help (ch, "motd");
   ...
   case CON_WIZ_INVIS:
      if (LOWER (*argument) == 'y')
      {
         ...
         stc ("#eLogging on in`G w`yi`Gz`y-`Gi`yn`Gv`yi`Gs`e.#n\n\r", ch);
//         do_help (ch, "motd");
         d->connected = CON_READ_MOTD;
	     do_function( d->character, &do_changes, "all" );
         return;
      }
      else if (LOWER (*argument) == 'n')
      {
         ...
         stc ("`eLogging on without`G w`yi`Gz`y-`Gi`yn`Gv`yi`Gs`e.#n\n\r", ch);
//         do_help (ch, "motd");
         d->connected = CON_READ_MOTD;
	     do_function( d->character, &do_changes, "all" );
         return;
         ...
         stc ("#eLog on in`G w`yi`Gz`y-`Gi`yn`Gv`yi`Gs`e mode? (`wy`e/`wn`e):`n", ch);
      return;


      write_to_buffer (d, "\n\r", 2);

      if (count_ability_levels (PHYSICAL, ch) + count_ability_levels (SOCIAL, ch) + count_ability_levels (MENTAL, ch) >= 27)
      {
         if (IS_IMMORTAL (ch))
            do_help (ch, "changes");
         else
//            do_help (ch, "motd");
      ...
      {
         send_to_char ("\n\r#R XP is made slowly, so choose carefully. View the 'help rec' helpfile after you create.#n\n\r", ch);
         send_to_char ("\n\r#nDo you want #g[#GP#g]#Ghysicals#n, #o[#yS#o]#yocials#n, or #e[#wM#e]#wentals#n\n\rto be your area of expertise? ", ch);
   ...
   case CON_ACCEPT_DISCLAIMER:
      ...
      {
      case 'a': case 'A':
         write_to_buffer( d, "#wWelcome to Innocence Lost, a modern World of Darkness 2E RPI.\n\r", 0);
         write_to_buffer (d, "#eWhat is your sex `e(`wM`e/`wF`e)?#n ", 0);
      ...
      default:
         write_to_buffer (d, "#nThis MUD has some rules you have to abide by to play.  Do you wish to #r(#Rr#r)#Read`n the rules,#r(#Ra#r)#Rgree`n or #r(#Rd#r)#Risagree`n to abide by them?`n", 0);
   ...
   case CON_GET_NEW_SEX:
      ...
      case 'F':
         ...
         break;
      default:
         write_to_buffer (d, "#eA gender please.. \n\rWhat #RIS#e your gender? ", 0);
         return;
      }
   ...
   case CON_GET_NEW_EMAIL:
   {
      ...
      if( argument[0] == '\0' )
      {
         write_to_buffer( d, "You MUST enter something, be it 'None' or your username.\n\r", 0);
         write_to_buffer( d, "Site username: ", 0 );
         d->connected = CON_GET_NEW_EMAIL;
         break;
      }   
   ...
   case CON_GET_NEW_ANSI:
      ...
      write_to_buffer (d, "\n\r#R Visit lishmud.com for assistance with the creation process.#n\n\r", 0);
      write_to_buffer (d, "\n\r#nDo you want `r[#RP#r]`rhysical`n, `c[#CS#c]`Cocial`n, or `e[#wM#e]`wental`n Attributes\n\rto be your area of expertise?", 0);
      break;
   ...
   case CON_GET_PRIMARY_STAT_AREA:
      ...
      case 'p':
      case 'P':
         write_to_buffer (d, "#nYou have chosen `rPhysical`n as your area of expertise.\n\r\n\r", 0);
         break;
      case 'm':
      case 'M':
         write_to_buffer (d, "#nYou have chosen`w Mental`n as your area of expertise.\n\r\n\r", 0);
         break;
      case 's':
      case 'S':
         write_to_buffer (d, "#nYou have chosen `CSocial`n as your area of expertise.\n\r\n\r", 0);
         break;
      default:
         write_to_buffer (d, "#RError#n:#n Please choose what your primary stat group will be\n\r(`r[#RP#r]`rhysical`n, `c[#CS#c]`Cocial`n, or `e[#wM#e]`wental#n)", 0);
         return;
      }
      ch->pcdata->stat_groups[0] = *argument;
      d->connected = CON_GET_SECONDARY_STAT_AREA;
      write_to_buffer (d, "#nDo you want `r[#RP#r]`rhysical`n, `c[#CS#c]`Cocial`n, or `e[#wM#e]`wental`n\n\r Attributes to be your secondary area of expertise? ", 0);
      break;
   ...
   case CON_GET_SECONDARY_STAT_AREA:
      ...
      case 'p':
      case 'P':
         write_to_buffer (d, "You have chosen `rPhysical`n as your secondary area of expertise.\n\r\n\r", 0);
         break;
      case 'm':
      case 'M':
         write_to_buffer (d, "You have chosen `wMental`n as your secondary area of expertise.\n\r\n\r", 0);
         break;
      case 'S':
      case 's':
         write_to_buffer (d, "You have chosen`C Social`n as your secondary area of expertise.\n\r\n\r", 0);
         break;
      default:
   ...
   /********* DISPLAY STATS ****************************/
      ...
      sprintf (tempbuf, "#e1)#n #GIncrease#n %-14s\t\t#e2)#n #RDecrease#n %-14s\n\r", attrib_names[j][0][0], attrib_names[j][0][0]);
      write_to_buffer (d, tempbuf, 0);
      sprintf (tempbuf, "#e3)#n #GIncrease#n %-14s\t\t#e4)#n #RDecrease#n %-14s\n\r", attrib_names[j][1][0], attrib_names[j][1][0]);
      write_to_buffer (d, tempbuf, 0);
      sprintf (tempbuf, "#e5)#n #GIncrease#n %-14s\t\t#e6)#n #RDecrease#n %-14s\n\r", attrib_names[j][2][0], attrib_names[j][2][0]);
      write_to_buffer (d, tempbuf, 0);
   ...
   case CON_GET_PRIMARY_STATS:
      ...
      case '1':
         ...
         {
            write_to_buffer (d, "That Attribute is already maxed.\n\r", 0);
            break;
         }
         ch->attributes[0 + 3 * j]++;
         break;
      case '2':
         ...
         {
            write_to_buffer (d, "You cannot be below 1 on any Attribute.\n\r", 0);
            break;
         }
         ch->attributes[0 + 3 * j]--;
         break;
      ...
      sprintf (tempbuf, "#w%s#n:#e[#n", attrib_names[j][2][0]);
      for (stat_display_counter = 0; stat_display_counter < ch->attributes[2 + 3 * j]; stat_display_counter++)
         strcat (tempbuf, "#Co#n");
      strcat (tempbuf, "#e]#n   ");
      write_to_buffer (d, tempbuf, 0);
      ...
      sprintf (tempbuf, "#e1)#n #GIncrease#n %-14s\t\t#e2) #RDecrease#n %-14s\n\r", attrib_names[j][0][0], attrib_names[j][0][0]);
      write_to_buffer (d, tempbuf, 0);
      sprintf (tempbuf, "#e3)#n #GIncrease#n %-14s\t\t#e4) #RDecrease#n %-14s\n\r", attrib_names[j][1][0], attrib_names[j][1][0]);
      write_to_buffer (d, tempbuf, 0);
      sprintf (tempbuf, "#e5)#n #GIncrease#n %-14s\t\t#e6) #RDecrease#n %-14s\n\r", attrib_names[j][2][0], attrib_names[j][2][0]);
      write_to_buffer (d, tempbuf, 0);
      if (points_distribution[0] - (ch->attributes[0 + 3 * j] + ch->attributes[1 + 3 * j] + ch->attributes[2 + 3 * j]) <= 0)
      {
         ...
         write_to_buffer (d, "Press #c<#CEnter#c>#n to continue\n\r\n\r", 0);
   ...
   case CON_GET_SECONDARY_STATS:
	  ...
	  ch->pcdata->willpower[WILLPOWER_CURRENT] = ((ch->attributes[ATTRIB_PER]) + (ch->attributes[ATTRIB_APP]) + 1);
      ...
      sprintf (tempbuf, "#e1)#G Increase#n %-14s\t\t2) #RDecrease#n %-14s\n\r", attrib_names[j][0][0], attrib_names[j][0][0]);
      write_to_buffer (d, tempbuf, 0);
      sprintf (tempbuf, "#e3)#G Increase#n %-14s\t\t4) #RDecrease#n %-14s\n\r", attrib_names[j][1][0], attrib_names[j][1][0]);
      write_to_buffer (d, tempbuf, 0);
      sprintf (tempbuf, "#e5)#G Increase#n %-14s\t\t6) #RDecrease#n %-14s\n\r", attrib_names[j][2][0], attrib_names[j][2][0]);
      write_to_buffer (d, tempbuf, 0);
   ...
   case CON_GET_PRIMARY_ABILITY_AREA:
      ...
      case 'p':
      case 'P':
         write_to_buffer (d, "You have chosen`G Physical Skills`n as your area of expertise.\n\r\n\r", 0);
         break;
      case 's':
      case 'S':
         write_to_buffer (d, "You have chosen`y Social Skills `nas your area of expertise.\n\r\n\r", 0);
         break;
      case 'm':
      case 'M':
         write_to_buffer (d, "You have chosen `wMental Skills`n as your area of expertise.\n\r\n\r", 0);
         break;
      default:
         write_to_buffer (d, "#RError#n: Please choose what your primary expertise group will be\n\r(#g[#GP#g]#Ghysicals#n, #o[#yS#o]#yocials#n, or #e[#wM#e]#wentals#n)", 0);
         return;
      }
   ...
   case CON_GET_SECONDARY_ABILITY_AREA:
      ...
      case 'p':
      case 'P':
         write_to_buffer (d, "You have chosen`G Physical Skills`n as your secondary area of expertise.\n\r\n\r", 0);
         break;
      case 's':
      case 'S':
         write_to_buffer (d, "You have chosen `ySocial Skills`n as your secondary area of expertise.\n\r\n\r", 0);
         break;
      case 'm':
      case 'M':
         write_to_buffer (d, "You have chosen`w Mental Skills `nas your secondary area of expertise.\n\r\n\r", 0);
         break;
      default:
         write_to_buffer (d, "#RError#n: Please choose what your secondary expertise group will be\n\r(#g[#GP#g]#Ghysicals#n, #o[#yS#o]#yocials#n, or #e[#wM#e]#wentals#n)", 0);
         return;
      }
      ch->pcdata->ability_groups[1] = *argument;
      if ((ch->pcdata->ability_groups[0] == 'p' && ch->pcdata->ability_groups[1] == 'm') || (ch->pcdata->ability_groups[0] == 'm' && ch->pcdata->ability_groups[1] == 'p'))
         ch->pcdata->ability_groups[2] = 's';
      else if ((ch->pcdata->ability_groups[0] == 'p' && ch->pcdata->ability_groups[1] == 's') || (ch->pcdata->ability_groups[0] == 's' && ch->pcdata->ability_groups[1] == 'p'))
         ch->pcdata->ability_groups[2] = 'm';
      else if ((ch->pcdata->ability_groups[0] == 's' && ch->pcdata->ability_groups[1] == 'm') || (ch->pcdata->ability_groups[0] == 'm' && ch->pcdata->ability_groups[1] == 's'))
         ch->pcdata->ability_groups[2] = 'p';
      sprintf (log_buf, "#RAREAS: %c  %c  %c.\n\r", ch->pcdata->ability_groups[0], ch->pcdata->ability_groups[1], ch->pcdata->ability_groups[2]);
      send_to_char (log_buf, ch);


      if (ch->pcdata->ability_groups[0] == 'p')
         j = PHYSICAL;
      if (ch->pcdata->ability_groups[0] == 's')
         j = SOCIAL;
      if (ch->pcdata->ability_groups[0] == 'm')
         j = MENTAL;
   /********* DISPLAY STATS ****************************/

      write_to_buffer (d, "#RThere is a bug in chargen - Don't use the 'decrease' functions in the Skill section or your points will disappear. We're working on it.#n\n\r#c------------------------------------------------------------------#n\r", 0);
      for (i = 0; i < 8; i++)
      {
         if (i % 3 == 0)
            send_to_char ("\n\r", ch);
         sprintf (tempbuf, "#w%-15s#n:#e[#n", ability_names[j][i][0]);
      ...
      for (i = 0; i < 8; i++)
      {
         sprintf (tempbuf, "#e%d)#G Increase#n %-14s\t\t#e%d) #RDecrease#n %-14s\n\r", i, ability_names[j][i][0], i + 8, ability_names[j][i][0]);
         send_to_char (tempbuf, ch);
      }
      send_to_char ("#c------------------------------------------------------------------#n\n\r", ch);
   ...
   case CON_GET_ABILITIES:
      if (ch->pcdata->ability_groups[0] == 'p')
         ...
         j = SOCIAL;
      else if (ch->pcdata->ability_groups[0] == 'm')
         j = MENTAL;
      ability_level = 0;

      if (count_ability_levels (j, ch) == 11)
      {
         if (ch->pcdata->ability_groups[1] == 'p')
            j = PHYSICAL;
         else if (ch->pcdata->ability_groups[1] == 's')
            j = SOCIAL;
         else if (ch->pcdata->ability_groups[1] == 'm')
            j = MENTAL;
         ability_level = 1;

         if (count_ability_levels (j, ch) == 7)
         {
            if (ch->pcdata->ability_groups[2] == 'p')
               j = PHYSICAL;
            else if (ch->pcdata->ability_groups[2] == 's')
               j = SOCIAL;
            else if (ch->pcdata->ability_groups[2] == 'm')
               j = MENTAL;
            ability_level = 2;

            if (count_ability_levels (j, ch) == 4)
            {
      ...
      else if ((argument[0] >= 48 && argument[0] <= 57) && (argument[1] >= 48 && argument[1] <= 57))
      {
         ...
         if (argument[0] == '0')
         {
            if (ch->abilities[j][temp_num] >= MAX_CHAR_CREATION_ABILITY)
               send_to_char ("#RError:#n You simply cannot go any higher with that ability.\n\r", ch);
            else
            {
         ...
         else if (argument[0] == '1')
         {
            if (ch->abilities[j][temp_num] <= 0)
               send_to_char ("#RError#n: How do you propose to have a negative ability?\n\r", ch);
            else
            {
               ch->abilities[j][temp_num] = ch->abilities[j][temp_num] - 1;
               ch->pcdata->abilities_points[j][temp_num] = ability_insight_levels[ch->abilities[j][temp_num]];
            ...
            send_to_char ("#RError#n: Try Again\n\r", ch);
      }
      else
         send_to_char ("#RError#n: Send the proper input.\n\r", ch);


      write_to_buffer (d, "#c------------------------------------------------------------------#n\r", 0);
      for (i = 0; i < 8; i++)
      {
         if (i % 3 == 0)
            send_to_char ("\n\r", ch);
         sprintf (tempbuf, "#w%-15s#n:#e[", ability_names[j][i][0]);
         for (stat_display_counter = 0; stat_display_counter < ch->abilities[j][i]; stat_display_counter++)
            strcat (tempbuf, "#Co");
         strcat (tempbuf, "#e]    ");
         send_to_char (tempbuf, ch);
      ...
      for (i = 0; i < 8; i++)
      {
         sprintf (tempbuf, "#e%d)#G Increase#n %-14s\t\t#e%d)#R Decrease#n %-14s\n\r", i, ability_names[j][i][0], i + 8, ability_names[j][i][0]);
         send_to_char (tempbuf, ch);
      }
      send_to_char ("#c------------------------------------------------------------------#n\n\r", ch);
      ...
      {
         write_to_buffer (d, "\n\r\n\rYou are out of points, proceeding to next Ability section\n\r", 0);
         write_to_buffer (d, "Press #c<#CEnter#c>#n to continue\n\r\n\r", 0);
      }
   ...
   case CON_READ_MOTD:
      ...
      if (IS_SET (ch->act, PLR_VANISH) && !IS_CHANGELING (ch))
      {
	...
	// ch->pcdata->auto_vote_counter = 1;
         ...
         set_title (ch, "is a new character!");
         ch->exp = 0;
         ...
         send_to_char ("#r--------------------------------------------------------------------------------#n\n\r", ch);
         send_to_char ("#eIf you need help, just ask over advice. See #whelp channels#n for details.#n\n\r", ch);
         send_to_char ("#r--------------------------------------------------------------------------------#n\n\r", ch);

         char_to_room (ch, get_room_index (ROOM_VNUM_SCHOOL));
         /*** Newbie Equipment ***/
		 obj_to_char( create_object(get_obj_index( 40 ), 1), ch );
//         obj_to_char( create_object(get_obj_index( 3355 ), 1), ch ); Necklace -- Must be purchased
...
//         obj_to_char( create_object(get_obj_index( 1010 ), 1), ch ); Tattoo -- Must be purchased
...
//         obj_to_char( create_object(get_obj_index( 3364 ), 1), ch ); Ring -- Must be purchased
         ...
         obj_to_char( create_object(get_obj_index( 1009 ), 1), ch );

		sprintf (buf, "#w%s #nis a new character. Please greet them over Advice!#r]#n#n", ch->name);
		do_info (ch, buf);
...
//      if ( ( ( get_age(ch) - 17 ) * 2) < 2 )
      ...
      //sprintf (buf, "#wSome new person #ehas entered Innocence Lost#n");
      //else
      //sprintf (buf, "#w%s #ehas entered Innocence Lost.#n", ch->name);
 
     if ( !IS_SET( ch->act, PLR_WIZINVIS))
      //   do_info (ch, buf);
      act ("#w$n#e has entered the game.#n", ch, NULL, NULL, TO_ROOM);
   ...
   for (ch = char_list; ch != NULL; ch = ch->next)
   {
      if (!IS_NPC (ch) && (!fConn || ch->desc == NULL) && !str_cmp (d->character->name, ch->name))
      {
         ...
         else
         {
            ...
            ch->desc = d;
            ch->timer = 0;
            send_to_char ("You reconnect.\n\r", ch);
            act ("$n reconnects.", ch, NULL, NULL, TO_ROOM);
            sprintf (log_buf, "%s reconnects.", ch->name);
            log_string (log_buf, CHANNEL_LOG_NORMAL);
            d->connected = CON_PLAYING;
         }
         return TRUE;
...
bool check_playing (DESCRIPTOR_DATA * d, char *name)
{
   ...
   for (dold = descriptor_list; dold != NULL; dold = dold->next)
   {
      if (dold != d && dold->character != NULL && dold->connected != CON_GET_NAME && dold->connected != CON_GET_OLD_PASSWORD && !str_cmp (name, dold->original ? dold->original->name : dold->character->name))
      {
         ...
         send_to_char ("You reconnect.\n\r", d->character);
         act ("$n reconnects.", d->character, NULL, NULL, TO_ROOM);
         act ("$n reconnects.", d->character, NULL, NULL, TO_ROOM);
         dold->character = NULL;
...
void stop_idling (CHAR_DATA * ch)
{
   ...
   ch->was_in_room = NULL;
   act ("$n is no longer idle.", ch, NULL, NULL, TO_ROOM);
   return;
}
...
void bust_a_prompt (DESCRIPTOR_DATA * d)
{
   ...
   while (*str != '\0')
   {
      ...
      case 'S':      break;
      case 't':
//         sprintf (buf2, "%d",    ch->pcdata->glamour[0]);
//         ADD_COLOUR (ch, buf2, L_CYAN);
//         i = buf2;
      break;
      case 'T':
//         sprintf (buf2, "%d",    ch->pcdata->glamour[1]);
//         ADD_COLOUR (ch, buf2, L_CYAN);
//         i = buf2;
      break;
      case 'u':
 //     sprintf (buf2, "%d", ch->pcdata->wyrd[0]);
//         ADD_COLOUR (ch, buf2, L_CYAN);
//         i = buf2;
      break;
      case 'U':
//         sprintf (buf2, "%d",    ch->pcdata->wyrd[1]);
//         ADD_COLOUR (ch, buf2, L_CYAN);
//         i = buf2;
      break;
      case 'v':
      sprintf (buf2, "%d", ch->pcdata->clarity[0]);
         ADD_COLOUR (ch, buf2, L_CYAN);
         i = buf2;
      break;
      case 'V':
         sprintf (buf2, "%d",    ch->pcdata->clarity[1]);
         ADD_COLOUR (ch, buf2, L_CYAN);
         i = buf2;
      ...
      case 'r':
         if (!IS_NPC(ch))
            {
               if (IS_MORE2 (ch, MORE2_RPFLAG))
                     strcpy (buf2, "#erpflag#n");
               else
                     strcpy (buf2, "#eno rpflag#n");
            }
         else
            strcpy (buf2, "n/a");
...
void do_login (CHAR_DATA * ch, char *argument)
{
   ...
   if (IS_SET (chars_logged_in, LOGIN_CHANGELING))
