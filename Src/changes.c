...
void do_addchange(CHAR_DATA * ch, char *argument)
{
        ...
        sprintf(buf,
                "#n%s change #R%d #nadded, type '#Rchanges#n' to see the details. #r]#n",
...
void do_delchange(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("#wFor delchange you must provide a change number.#e\n\r",
                         ch);
...
void do_changes(CHAR_DATA * ch, char *argument)
{
        ...
        snprintf(buf, MSL, "#w Innocence Lost RPI #gchangelog");
...
/*        if (is_number(arg))
        {
                ...
                {
                        sprintf(buf, "           #RPage must be between 1 and %d!!!\n\r", totalpages);
                        send_to_char(buf, ch);
                        return;
                }
                ...
                {
                        sprintf(buf, "#0[#R%3d#0] %-11s #g*%-6s #P%-5s #w%-45s#D\n\r",
...
*/
                ...
                {
                        sprintf(buf, "#w%3d) %-11s #g*%-8s #G%-5s #n%-45s\n\r",
                                (i + 1), changes_table[i].coder,
                                changes_table[i].date, changes_table[i].type,
        ...
        send_to_char
                ("#g--------------------------------------------------------------------------------#n\n\r",
                 ch);
        sprintf(buf,
                "#n There are #g<#w%d#g> #nchanges total, %d added today. Filter by \"change <type>\".\n\r",
                maxChanges, today);
        send_to_char(buf, ch);
        if (totalpages > 1)
        {
                sprintf(buf,
                        "#0             Or: '#Gchanges <1 to %d>#0' for individual pages.#n\n\r",
                        totalpages);
                send_to_char(buf, ch);
        }
        //send_to_char("#0             Or: '#Gchanges <page number>#0' for individual page.#n\n\r",ch);
        send_to_char
                ("#g--------------------------------------------------------------------------------#n\n\r",
                 ch);
        return;
