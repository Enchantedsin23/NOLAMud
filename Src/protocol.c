...
static void Write( DESCRIPTOR_DATA *apDescriptor, const char *apData )
{
...
static void InfoMessage( DESCRIPTOR_DATA *apDescriptor, const char *apData )
{
   Write( apDescriptor, "\t[F210][\toINFO\t[F210]]\tn " );
   Write( apDescriptor, apData );
}

static void CompressStart( DESCRIPTOR_DATA *apDescriptor )
{
...
static void CompressEnd( DESCRIPTOR_DATA *apDescriptor )
{
   /* If your mud uses MCCP (Mud Client Compression Protocol), you need to 
    * call whatever function normally starts compression from here - the 
    * ReportBug() call should then be deleted.
...
static variable_name_t VariableNameTable[eMSDP_MAX+1] = 
{
   ...
   { eMSDP_XTERM_256_COLORS, "XTERM_256_COLORS", BOOLEAN_SET_TO(1) }, 
   { eMSDP_UTF_8,            "UTF_8",            BOOLEAN_SET_TO(1) }, 
   { eMSDP_SOUND,            "SOUND",            BOOLEAN_SET_TO(0) }, 
   { eMSDP_MXP,              "MXP",              BOOLEAN_SET_TO(1) }, 
...
static void Negotiate               ( DESCRIPTOR_DATA *apDescriptor );
static void PerformHandshake        ( DESCRIPTOR_DATA *apDescriptor, char aCmd, char aProtocol );
static void PerformSubnegotiation   ( DESCRIPTOR_DATA *apDescriptor, char aCmd, char *apData, int aSize );
static void SendNegotiationSequence ( DESCRIPTOR_DATA *apDescriptor, char aCmd, char aProtocol );
static bool_t ConfirmNegotiation    ( DESCRIPTOR_DATA *apDescriptor, negotiated_t aProtocol, bool_t abWillDo, bool_t abSendReply );

static void ParseMSDP               ( DESCRIPTOR_DATA *apDescriptor, const char *apData );
static void ExecuteMSDPPair         ( DESCRIPTOR_DATA *apDescriptor, const char *apVariable, const char *apValue );

static void ParseATCP               ( DESCRIPTOR_DATA *apDescriptor, const char *apData );
#ifdef MUDLET_PACKAGE
static void SendATCP                ( DESCRIPTOR_DATA *apDescriptor, const char *apVariable, const char *apValue );
#endif /* MUDLET_PACKAGE */

static void SendMSSP                ( DESCRIPTOR_DATA *apDescriptor );
...
void ProtocolInput( DESCRIPTOR_DATA *apDescriptor, char *apData, int aSize, char *apOut )
{
...
const char *ProtocolOutput( DESCRIPTOR_DATA *apDescriptor, const char *apData, int *apLength )
{
 ...
 */
void ProtocolNegotiate( DESCRIPTOR_DATA *apDescriptor )
{
   ConfirmNegotiation(apDescriptor, eNEGOTIATED_TTYPE, true, true);
}

/* Tells the client to switch echo on or off. */
void ProtocolNoEcho( DESCRIPTOR_DATA *apDescriptor, bool_t abOn )
{
...
const char *CopyoverGet( DESCRIPTOR_DATA *apDescriptor )
{
...
void CopyoverSet( DESCRIPTOR_DATA *apDescriptor, const char *apData )
{
   protocol_t *pProtocol = apDescriptor ? apDescriptor->pProtocol : NULL;
...
void MSDPUpdate( DESCRIPTOR_DATA *apDescriptor )
{
   int i; /* Loop counter */
...
void MSDPFlush( DESCRIPTOR_DATA *apDescriptor, variable_t aMSDP )
{
...
void MSDPSend( DESCRIPTOR_DATA *apDescriptor, variable_t aMSDP )
{
...
void MSDPSendPair( DESCRIPTOR_DATA *apDescriptor, const char *apVariable, const char *apValue )
{
   char MSDPBuffer[MAX_VARIABLE_LENGTH+1] = { '\0' };
...
void MSDPSendList( DESCRIPTOR_DATA *apDescriptor, const char *apVariable, const char *apValue )
{
   char MSDPBuffer[MAX_VARIABLE_LENGTH+1] = { '\0' };
...
void MSDPSetNumber( DESCRIPTOR_DATA *apDescriptor, variable_t aMSDP, int aValue )
{
   protocol_t *pProtocol = apDescriptor ? apDescriptor->pProtocol : NULL;
...
void MSDPSetString( DESCRIPTOR_DATA *apDescriptor, variable_t aMSDP, const char *apValue )
{
   protocol_t *pProtocol = apDescriptor ? apDescriptor->pProtocol : NULL;
...
void MSDPSetTable( DESCRIPTOR_DATA *apDescriptor, variable_t aMSDP, const char *apValue )
{
   protocol_t *pProtocol = apDescriptor ? apDescriptor->pProtocol : NULL;
...
void MSDPSetArray( DESCRIPTOR_DATA *apDescriptor, variable_t aMSDP, const char *apValue )
{
   protocol_t *pProtocol = apDescriptor ? apDescriptor->pProtocol : NULL;
...
const char *MXPCreateTag( DESCRIPTOR_DATA *apDescriptor, const char *apTag )
{
   protocol_t *pProtocol = apDescriptor ? apDescriptor->pProtocol : NULL;
...
void MXPSendTag( DESCRIPTOR_DATA *apDescriptor, const char *apTag )
{
   protocol_t *pProtocol = apDescriptor ? apDescriptor->pProtocol : NULL;
...
void SoundSend( DESCRIPTOR_DATA *apDescriptor, const char *apTrigger )
{
...
const char *ColourRGB( DESCRIPTOR_DATA *apDescriptor, const char *apRGB )
{
   protocol_t *pProtocol = apDescriptor ? apDescriptor->pProtocol : NULL;
...
static void SendMSSP( DESCRIPTOR_DATA *apDescriptor )
{
   ...
   static MSSP_t MSSPTable[] =
   {
      ...
      { "HOSTNAME",           "lishmud.com" },
      { "PORT",               "3660" },
      { "CODEBASE",           "Heavily-Modified Godwars" },
      { "CONTACT",            "enigma@lishmud.com" },
      { "CREATED",            "2016" },
      { "ICON",               "http://i.imgur.com/HypHzKj.png" },
      { "IP",                 "66.85.147.90" },
      { "LANGUAGE",           "English" },
      { "LOCATION",           "USA" },
      { "MINIMUM AGE",        "18" },
