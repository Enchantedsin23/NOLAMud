...
#define IS_BUILDER2(ch, Area)  ( GET_PC(ch,security,8) <= Area->security              \
...
MOB_INDEX_DATA *new_mob_index (void)
{
	...
	// Gobbo Trial Code *****
	pMob->attributes[ATTRIB_STR] = 1;
	pMob->attributes[ATTRIB_DEX] = 1;
	pMob->attributes[ATTRIB_STA] = 1;

	pMob->attributes[ATTRIB_CHA] = 1;
	pMob->attributes[ATTRIB_MAN] = 1;
	pMob->attributes[ATTRIB_APP] = 1;

	pMob->attributes[ATTRIB_PER] = 1;
	pMob->attributes[ATTRIB_INT] = 1;
	pMob->attributes[ATTRIB_WIT] = 1;

	for (i = 0; i < 3; i++)
		for (j = 0; j < 10; j++)
			pMob->abilities[i][j] = 0;
...
void aedit (CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp (arg1, "done"))
	{
		ch->desc->pEdit = NULL;
		ch->desc->editor = 0;
		send_to_char ("Exiting Editor.", ch);
	...
	if (!str_cmp (arg1, "lvnum"))
	{
		...
		if (get_vnum_area (value) != NULL && get_vnum_area (value) != pArea)
		{
...
int get_exit_flag_number (char *arg)
{
	...
	if (!str_cmp (arg, "bashproof"))
		return EX_BASHPROOF;
	if (!str_cmp (arg, "changeling"))
		return EX_CHANGELING;
...
char *get_exit_flag_string (int exit)
{
	...
	if (exit & EX_CHANGELING)
		strcat (buf, " changeling");
...
void redit (CHAR_DATA * ch, char *argument)
{
	...
	if ((door = get_direction_number (arg1)) != MAX_DIR && arg2[0] != '\0')
	{
		...
		if (!str_cmp (arg1, "link"))
		{
			...
			if (!IS_BUILDER2 (ch, get_room_index (value)->area))
			{
...
void show_obj_values (CHAR_DATA * ch, OBJ_INDEX_DATA * obj)
{
	...
	case ITEM_STAFF:
		sprintf (buf, "(v3) %s\n\rLevel (v0): %d\n\rCharges (v2)/(v1): %d / %d\n\r", obj->value[3] != -1 ? capitalize (skill_table[obj->value[3]].name) : "!NO SPELL!", obj->value[0], obj->value[2], obj->value[1]);
		send_to_char (buf, ch);
		break;
	...
	case ITEM_POTION:
	case ITEM_SCENT:
		sprintf (buf, "(v1) %s\n\r(v2) %s\n\r(v3) %s\n\rLevel (v0): %d\n\r", obj->value[1] != -1 ? capitalize (skill_table[obj->value[1]].name) : "!NO SPELL!", obj->value[2] != -1 ? skill_table[obj->value[2]].name : "!NO SPELL!", obj->value[3] != -1 ? skill_table[obj->value[3]].name : "!NO SPELL!", obj->value[0]);
		send_to_char (buf, ch);
		break;

	case ITEM_WEAPON:
		sprintf (buf, "Type(v3): %s\n\rMin dam(v1): %d\n\rMax dam(v2): %d\n\r", get_type_weapon (obj->value[3]), obj->value[1], obj->value[2]);
		send_to_char (buf, ch);
		break;

		// Gobbo adding armor
	case ITEM_ARMOR:
		sprintf (buf, "Stamina Bonus for armor(v0): %d\n\rDexterity Penalty for armor(v1): %d\n\r", obj->value[0], obj->value[1]);
		send_to_char (buf, ch);
		break;
		
	case ITEM_VEHICLE:
//		sprintf (buf, "Vnum you'll go to(v0): %d\n\rNumber of uses(v1): %d\n\rSet to 1(v2): %d\n\rThe room the vehicle is currently in(v3): %d\n\r", obj->value[0], obj->value[1], obj->value[2], obj->value[3]);
		sprintf (buf, "Inside description: %s\n\rOutside description: %s", obj->inside_desc, obj->outside_desc );
		send_to_char (buf, ch);
		break;

	case ITEM_CONTAINER:
		sprintf (buf, "Weight (v0): %d \n\rFlags (v1): %s\nrKey (v2): %s (%d)\n\r", obj->value[0], get_flags_container (obj->value[1]), get_obj_index (obj->value[2]) ? get_obj_index (obj->value[2])->short_descr : "none", obj->value[2]);
		send_to_char (buf, ch);
		break;

	case ITEM_DRINK_CON:
		sprintf (buf, "Contains (v1)/(v0): %d/%d\n\rLiquid (v2): %s\n\rPoisoned? (v3): %s\n\r", obj->value[1], obj->value[0], get_type_liquid (obj->value[2]), obj->value[3] != 0 ? "Yes" : "No");
		send_to_char (buf, ch);
		break;
	...
	case ITEM_MONEY:
		sprintf (buf, "Number of Pounds (v0): %d\n\r", obj->value[0]);
		send_to_char (buf, ch);
		break;
	}
...
void set_obj_values (CHAR_DATA * ch, OBJ_INDEX_DATA * pObj, int value_num, char *argument)
{
	...
	case ITEM_ARMOR:
		...
		default:
			...
			break;
		case 0:
			send_to_char ("ARMOR STAMINA BONUS SET.\n\r\n\r", ch);
			pObj->value[0] = atoi (argument);
			break;
		case 1:
			send_to_char ("ARMOR DEXTERITY PENALTY SET.\n\r\n\r", ch);
			pObj->value[1] = atoi (argument);
			break;
		}
		break;
	...
	case ITEM_VEHICLE:
		...
		case 1:
			...
			break;
		case 2:
			send_to_char ("CANNOT BE ENTERED IF 2.\n\r\n\r", ch);
			pObj->value[2] = atoi (argument);
			break;
		case 3:
			send_to_char ("ROOM VEHICLE IS IN SET.\n\r\n\r", ch);
	...
	case ITEM_MONEY:
		...
		default:
			...
			break;
		case 0:
			send_to_char ("POUNDS AMOUNT SET.\n\r\n\r", ch);
			pObj->value[0] = atoi (argument);
			break;
		}
		break;
...
void oedit (CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp (arg1, "outside") && (pObj->item_type == ITEM_VEHICLE))
	{
		if (arg2[0] == '\0')
		{
			send_to_char ("Syntax:  outside [string]\n\r", ch);
			return;
		}

		strcat (arg2, "\n\r");

		free_string (pObj->outside_desc);
		pObj->outside_desc = str_dup (arg2);

		send_to_char ("Outside description set.\n\r", ch);
		return;
	}

	if (!str_cmp (arg1, "inside") && (pObj->item_type == ITEM_VEHICLE))
	{
		if (arg2[0] == '\0')
		{
			send_to_char ("Syntax:  inside [string]\n\r", ch);
			return;
		}

		strcat (arg2, "\n\r");

		free_string (pObj->inside_desc);
		pObj->inside_desc = str_dup (arg2);

		send_to_char ("Inside description set.\n\r", ch);
		return;
	}
...
void medit (CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg1, "mobprog") || !str_cmp(arg1, "mp"))
	{
		argument = one_argument (argument, arg2);
		...
		if (arg2[0] == '\0')
		{
			...
			strcat(buf, MOB_DIR);
			send_to_char("#R------------------------------------------------------------------#n\n\r", ch);
			send_to_char("#R|#n Mob Programs                                                   #R|#n\n\r", ch);
			send_to_char("#R------------------------------------------------------------------#n\n\r", ch);
			send_to_char("#RSyntax:#n mobprog <mobfilename>\n\r", ch);
			send_to_char("#RExample:#n mobprog beggar	// Gives beggar mobprog\n\r", ch);
			send_to_char("#RExample:#n mobprog none	// Removes mobprog from mob\n\r", ch);
			send_to_char("#RPossible MobPrograms:#n\n\r", ch);
			strcpy(buf, "./");
			strcat(buf, MOB_DIR);
			dirp = opendir(buf);
		...
		if (!strcmp(arg2, "none"))
		{
			MPROG_RESETS *previous;
			MPROG_RESETS *current;
			send_to_char("Clearing mobprog", ch);
			current = pMob->area->mprogs;
			if(pMob->mobprog_filename == NULL)
				return;
			if(!strcmp(current->filename, pMob->mobprog_filename) && (current->vnum == pMob->vnum))
			{
				previous = current;
				pMob->area->mprogs = current->next;
				free_mem(previous, sizeof(MPROG_RESETS));
			}
			else
			{
				previous = pMob->area->mprogs;
				for(current=pMob->area->mprogs->next;current != NULL; current = current->next)
				{
					if(!strcmp(current->filename, pMob->mobprog_filename) && (current->vnum == pMob->vnum))
					{
						previous->next = current->next;
						free_mem(current, sizeof(MPROG_RESETS));
						break;
					}	
			...
			free_string(pMob->mobprog_filename);
			pMob->mobprog_filename = NULL;
			if(pMob->mobprogs != NULL)
			{
				free_mem(pMob->mobprogs, sizeof(MPROG_DATA));
				pMob->progtypes = 0;
			}
		}
		else
		{
			...
			if (dup == FALSE)
			{
				send_to_char("Error, mobprog not found.", ch);
				return;
			}
			else
			{
				...
				pMob->mobprog_filename = str_dup(arg2);
				send_to_char("Changing mobprog.", ch);
			//	temp = alloc_perm(sizeof(MPROG_RESETS));
			//	temp->vnum = pMob->vnum;
				strcat(arg2, ".prg");
			//	temp->filename = str_dup(arg2);
			//	temp->next = NULL;
				if(pMob->mobprogs != NULL)
				{
					free_mem(pMob->mobprogs, sizeof(MPROG_DATA));
				...
				pMob->mobprogs = working;
				working = mprog_file_read(arg2, working, pMob);
				working->next = NULL;

//				if (pMob->area->mprogs == NULL)
//					pMob->area->mprogs = temp;
//				else
//				{
//					temp->next = pMob->area->mprogs;
//					pMob->area->mprogs = temp;
//				}
	...
	if (!str_cmp (arg1, "str"))
	{
		...
		pMob->attributes[ATTRIB_STR] = atoi (arg2);
	...
	if (!str_cmp (arg1, "dex"))
	{
		...
		pMob->attributes[ATTRIB_DEX] = atoi (arg2);
	...
	if (!str_cmp (arg1, "sta"))
	{
		if (arg2[0] == '\0' || !is_number (arg2))
		{
			send_to_char ("Syntax:  sta [number]\n\r", ch);
			return;
		}

		pMob->attributes[ATTRIB_STA] = atoi (arg2);

		SET_BIT (pArea->area_flags, AREA_CHANGED);
		send_to_char ("Stamina set.\n\r", ch);
		return;
	}
	...
	if (!str_cmp (arg1, "cha"))
	{
		...
		pMob->attributes[ATTRIB_CHA] = atoi (arg2);

		SET_BIT (pArea->area_flags, AREA_CHANGED);
		send_to_char ("Charisma set.\n\r", ch);
		return;
	}
	...
	if (!str_cmp (arg1, "man"))
	{
		if (arg2[0] == '\0' || !is_number (arg2))
		{
			send_to_char ("Syntax:  man [number]\n\r", ch);
			return;
		}

		pMob->attributes[ATTRIB_MAN] = atoi (arg2);

		SET_BIT (pArea->area_flags, AREA_CHANGED);
		send_to_char ("Manipulation set.\n\r", ch);
		return;
	}
	...
	if (!str_cmp (arg1, "app"))
	{
		if (arg2[0] == '\0' || !is_number (arg2))
		{
			send_to_char ("Syntax:  app [number]\n\r", ch);
			return;
		}

		pMob->attributes[ATTRIB_APP] = atoi (arg2);
	...
	if (!str_cmp (arg1, "per"))
	{
		if (arg2[0] == '\0' || !is_number (arg2))
		{
			send_to_char ("Syntax:  per [number]\n\r", ch);
			return;
		}

		pMob->attributes[ATTRIB_PER] = atoi (arg2);

		SET_BIT (pArea->area_flags, AREA_CHANGED);
		send_to_char ("Perception set.\n\r", ch);
		return;
	}
	...
	if (!str_cmp (arg1, "int"))
	{
		...
		pMob->attributes[ATTRIB_INT] = atoi (arg2);
	...
	if (!str_cmp (arg1, "wit"))
	{
		if (arg2[0] == '\0' || !is_number (arg2))
		{
			send_to_char ("Syntax:  wit [number]\n\r", ch);
			return;
		}

		pMob->attributes[ATTRIB_WIT] = atoi (arg2);

		SET_BIT (pArea->area_flags, AREA_CHANGED);
		send_to_char ("Wit set.\n\r", ch);
		return;
	}

	if (!str_cmp (arg1, "brawl"))
	{
		if (arg2[0] == '\0' || !is_number (arg2))
		{
			send_to_char ("Syntax:  brawl [number]\n\r", ch);
			return;
		}

		pMob->abilities[PHYSICAL][TAL_BRAWL] = atoi (arg2);

		SET_BIT (pArea->area_flags, AREA_CHANGED);
		send_to_char ("Brawl set.\n\r", ch);
		return;
	}
	...
	if (!str_cmp (arg1, "stealth"))
	{
		if (arg2[0] == '\0' || !is_number (arg2))
		{
			send_to_char ("Syntax: stealth [number]\n\r", ch);
			return;
		}

		pMob->abilities[PHYSICAL][TAL_STEALTH] = atoi (arg2);

		SET_BIT (pArea->area_flags, AREA_CHANGED);
		send_to_char ("Stealth set.\n\r", ch);
		return;
	}

	if (!str_cmp (arg1, "streetwise"))
	{
		if (arg2[0] == '\0' || !is_number (arg2))
		{
			send_to_char ("Syntax:  streetwise [number]\n\r", ch);
			return;
		}

		pMob->abilities[SOCIAL][SKI_STREETWISE] = atoi (arg2);

		SET_BIT (pArea->area_flags, AREA_CHANGED);
		send_to_char ("Streetwise set.\n\r", ch);
		return;
	}
	...
	if (!str_cmp (arg1, "animal"))
	{
		if (IS_SET (pMob->act, ACT_ANIMAL))
			REMOVE_BIT (pMob->act, ACT_ANIMAL);
		else
			SET_BIT (pMob->act, ACT_ANIMAL);
		SET_BIT (pArea->area_flags, AREA_CHANGED);
		send_to_char ("Animal flag toggled.\n\r", ch);
	...
	if (!str_cmp (arg1, "hp"))
	{
		...
		pMob->level = atoi (arg2);
		pMob->max_hit = atoi (arg2);
		
		SET_BIT (pArea->area_flags, AREA_CHANGED);
		send_to_char ("Health level set.\n\r", ch);
		return;
	}
...
void do_aedit (CHAR_DATA * ch, char *argument)
{
	...
	ch->desc->pEdit = (void *) pArea;
	ch->desc->editor = ED_AEDITOR;
	send_to_char ("\n\r Area Editor Entered.", ch);
	return;
}
...
void do_oedit (CHAR_DATA * ch, char *argument)
{
	...
	if (is_number (arg1))
	{
		...
		ch->desc->pEdit = (void *) pObj;
		ch->desc->editor = ED_OEDITOR;
		send_to_char ("\n\r Object Editor Entered.", ch);
		return;
	}
	else
	{
...
void do_medit (CHAR_DATA * ch, char *argument)
{
	...
	if (is_number (arg1))
	{
		...
		ch->desc->pEdit = (void *) pMob;
		ch->desc->editor = ED_MEDITOR;
		send_to_char ("\n\r Mob Editor Entered.", ch);
...
void do_rshow (CHAR_DATA * ch, char *argument)
{
	...
	sprintf (buf, "#c.-------------- #nEntering#w Room#n Editor#c -----------------.#n\n\r");
	strcat (buf1, buf);

	sprintf (buf, "#y  •#C Name#G: '#n%s#w'#n\n\r#y  • #CArea#G:#o [#y%3d#o]#w '#n%s#w'#n\n\r", location->name, location->area->vnum, location->area->name);
	strcat (buf1, buf);

	sprintf (buf, "#y  •#C Vnum#G:#n %d#n\n\r#y  •#C Sector#G:#n %s#n\n\r#y  • #CLight#G:#n %d#n\n\r", location->vnum, sector_name (location->sector_type), location->light);
	strcat (buf1, buf);

	sprintf (buf, "#y  • #CRoom flags#G:#n %s#n\n\r#y  •#C Description#G:#n\n\r#n%s#n", room_bit_name (location->room_flags), location->description);
	strcat (buf1, buf);
	...
	if (location->extra_descr)
	{
		...
		strcat (buf1, "#CExtra description keywords#G:#n ");
	...
	strcat (buf1, "#y  •#C Characters#G:#n");
	...
	for (door = 0; door < MAX_DIR; door++)
	{
		...
		{
			sprintf (buf, "#c|------------------------#nExit#c-------------------------|#n\n\r");
			strcat (buf1, buf);
			sprintf (buf, "#C  Door#G:#n %5s#n   #CTo#C:#n %d#y   Key#o:#n %d#n#C  Exit flags#G:#R %d#r '#R%s#r'#n\n\r", dir_name[door], pexit->to_room ? pexit->to_room->vnum : 0, pexit->key, pexit->exit_info, (pexit->keyword != NULL) ? pexit->keyword : "");
			strcat (buf1, buf);
			if (pexit->description != NULL && pexit->description[0] != '\0')
			{
				sprintf (buf, "#R%s#n", pexit->description);
				strcat (buf1, buf);
			}
		}
	}
...
void do_mshow (CHAR_DATA * ch, char *argument)
{
	...
	sprintf (buf, "Name: '%s'\n\rArea %d: '%s'\n\r", victim->player_name, victim->area == NULL ? -1 : victim->area->vnum, victim->area == NULL ? "No Area" : victim->area->name);
	send_to_char (buf, ch);

	sprintf (buf, "Act: %s\n\r", act_bit_name (victim->act));
	send_to_char (buf, ch);

	if(victim->mobprog_filename != NULL)
	{
		sprintf( buf, "MobProg: %s\n\r", victim->mobprog_filename);
		send_to_char(buf, ch);
	}
	else
		send_to_char("MobProg:\n\r", ch);
	
	sprintf (buf, "Vnum: %d\n\rSex:  %s\n\r", victim->vnum, victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral");
	send_to_char (buf, ch);

	sprintf (buf, "Level: %d\n\rAlign: %d\n\r", victim->level, victim->alignment);
	send_to_char (buf, ch);

	sprintf (buf, "Affected by: %s\n\r", affect_bit_name (victim->affected_by));
	send_to_char (buf, ch);

	sprintf (buf, "Short description: %s\n\rLong description:\n\r%s", victim->short_descr, victim->long_descr);
	send_to_char (buf, ch);
	...
	if (victim->pShop != NULL)
	{
		...
		for (iTrade = 0; iTrade < MAX_TRADE; iTrade++)
		{
			...
			{
				sprintf (buf, "    Buys %ss (#%d)\n\r", item_type_name (pShop->buy_type[iTrade]), iTrade);
				send_to_char (buf, ch);
			}
		}
	}

	sprintf (buf, "Strength:     %d ", victim->attributes[0]);
	send_to_char (buf, ch);
	sprintf (buf, "\tCharisma:     %d", victim->attributes[3]);
	send_to_char (buf, ch);
	sprintf (buf, "\t\tPerception:   %d\n\r", victim->attributes[6]);
	send_to_char (buf, ch);

	sprintf (buf, "Dexterity:    %d ", victim->attributes[1]);
	send_to_char (buf, ch);
	sprintf (buf, "\tManipulation: %d", victim->attributes[4]);
	send_to_char (buf, ch);
	sprintf (buf, "\t\tIntelligence: %d\n\r", victim->attributes[7]);
	send_to_char (buf, ch);

	sprintf (buf, "Stamina:      %d ", victim->attributes[2]);
	send_to_char (buf, ch);
	sprintf (buf, "\tAppearance:    %d", victim->attributes[5]);
	send_to_char (buf, ch);
	sprintf (buf, "\t\tWit:          %d\n\r", victim->attributes[8]);
	send_to_char (buf, ch);

	sprintf (buf, "\nBrawl:   %d", victim->abilities[PHYSICAL][TAL_BRAWL]);
	send_to_char (buf, ch);
	sprintf (buf, "\tStealth: %d", victim->abilities[PHYSICAL][TAL_STEALTH]);
	send_to_char (buf, ch);
	sprintf (buf, "\tStreetwise:   %d\n\r", victim->abilities[SOCIAL][SKI_STREETWISE]);
	send_to_char (buf, ch);
...
void do_oshow (CHAR_DATA * ch, char *argument)
{
	...
	sprintf (buf, "Name: '%s'\n\rArea %d: '%s'\n\r", obj->name, obj->area == NULL ? -1 : obj->area->vnum, obj->area == NULL ? "No Area" : obj->area->name);
	send_to_char (buf, ch);
...
void display_resets (CHAR_DATA * ch)
{
	...
	for (pReset = pArea->reset_first; pReset; pReset = pReset->next)
	{
		...
		case 'M':
			...
			pMob = pMobIndex;
			sprintf (buf, "Loads %s (%d) in %s (%d) (max %d)", pMob->short_descr, pReset->arg1, pRoomIndex->name, pReset->arg3, pReset->arg2);
			strcat (final, buf);
		...
		case 'O':
			...
			sprintf (buf, "   Loads %s (%d) in %s (%d)\n\r", pObj->short_descr, pReset->arg1, pRoomIndex->name, pReset->arg3);
			strcat (final, buf);
		...
		case 'P':
			...
			sprintf (buf, "   Loads %s (%d) inside %s (%d)\n\r", pObj->short_descr, pReset->arg1, pObjToIndex->short_descr, pReset->arg3);
			strcat (final, buf);
		...
		case 'E':
			...
			if (pMob->pShop)
			{
				sprintf (buf, "   Loads %s (%d) in the inventory of %s (%d) shopkeeper\n\r", pObj->short_descr, pReset->arg1, pMob->short_descr, pMob->vnum);
			}
			else
				sprintf (buf, "   Loads %s (%d) %s of %s (%d)\n\r", pObj->short_descr, pReset->arg1, (pReset->command == 'G') ? wear_loc_name (WEAR_NONE) : wear_loc_name (pReset->arg3), pMob->short_descr, pMob->vnum);
			strcat (final, buf);
		...
		case 'D':
			...
			sprintf (buf, "The %s door of %s is reset to %s\n", dir_name[pReset->arg2], pRoomIndex->name, get_exit_flag_string (pReset->arg3));
			break;
...
void do_resets (CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_BUILDER2 (ch, ch->in_room->area))
	{
	...
	if (is_number (arg1))
	{
		...
		if (!str_cmp (arg2, "delete"))
		{
			...
			else
			{
				...
				for (pArea->reset_last = pArea->reset_first; pArea->reset_last->next != NULL; pArea->reset_last = pArea->reset_last->next);
			}
	...
	for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
	{
		for (pMobIndex = mob_index_hash[iHash]; pMobIndex != NULL; pMobIndex = pMobIndex->next)
		{
			if (pMobIndex != NULL && pMobIndex->area == pArea)
			{
				...
				fprintf (fp, "%dd%d+%d ", pMobIndex->hitnodice, pMobIndex->hitsizedice, pMobIndex->hitplus);
				fprintf (fp, "%dd%d+%d\n", pMobIndex->damnodice, pMobIndex->damsizedice, pMobIndex->damplus);
				fprintf (fp, "%d ", pMobIndex->pounds);
				fprintf (fp, "0\n0 0 ");
				fprintf (fp, "%d\n", pMobIndex->sex);

				fprintf (fp, "%d ", pMobIndex->attributes[0]);	// STR
				fprintf (fp, "%d ", pMobIndex->attributes[1]);	// DEX
				fprintf (fp, "%d \n", pMobIndex->attributes[2]);	// CON

				fprintf (fp, "%d ", pMobIndex->attributes[3]);	// CHA
				fprintf (fp, "%d ", pMobIndex->attributes[4]);	// MAN
				fprintf (fp, "%d \n", pMobIndex->attributes[5]);	// APP

				fprintf (fp, "%d ", pMobIndex->attributes[6]);	// PER
				fprintf (fp, "%d ", pMobIndex->attributes[7]);	// INT
				fprintf (fp, "%d \n", pMobIndex->attributes[8]);	// WIT

				fprintf (fp, "%d ", pMobIndex->abilities[PHYSICAL][TAL_BRAWL]);
				fprintf (fp, "%d ", pMobIndex->abilities[PHYSICAL][TAL_STEALTH]);
				fprintf (fp, "%d \n", pMobIndex->abilities[SOCIAL][SKI_STREETWISE]);
	...
	for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
	{
		for (pObjIndex = obj_index_hash[iHash]; pObjIndex != NULL; pObjIndex = pObjIndex->next)
		{
			if (pObjIndex != NULL && pObjIndex->area == pArea)
			{
				...
				case ITEM_POTION:
				case ITEM_SCROLL:
					fprintf (fp, "%d %d %d %d\n", pObjIndex->value[0], sn_lookup (pObjIndex->value[1]), sn_lookup (pObjIndex->value[2]), sn_lookup (pObjIndex->value[3]));
					break;
				case ITEM_STAFF:
				case ITEM_PHONE:
					fprintf (fp, "%d %d %d %d\n", pObjIndex->value[0], pObjIndex->value[1], pObjIndex->value[2], sn_lookup (pObjIndex->value[3]));
					break;
				}
				...
				if (pObjIndex->item_type == ITEM_VEHICLE)
				{
					fprintf (fp, "V\n%s~\n%s~\n", pObjIndex->inside_desc, pObjIndex->outside_desc);
				}
				if (pObjIndex->spectype > 0)
				{
					fprintf (fp, "Q\n%s~\n%s~\n%s~\n%s~\n%s~\n%s~\n%d %d\n", pObjIndex->inside_desc, pObjIndex->outside_desc, pObjIndex->chpoweruse, pObjIndex->victpoweron, pObjIndex->victpoweroff, pObjIndex->victpoweruse, pObjIndex->spectype, pObjIndex->specpower);
				}
			}
		}
	}
	...
	for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
	{
		for (pRoomIndex = room_index_hash[iHash]; pRoomIndex; pRoomIndex = pRoomIndex->next)
		{
			if (pRoomIndex->area == pArea)
			{
				...
				/* Change this version number as new items are added */
				// 1
				fprintf (fp, "%d\n", 1);
				fprintf (fp, "%d %d\n", pRoomIndex->x, pRoomIndex->y);
				/* End new version items */
				for (pEd = pRoomIndex->extra_descr; pEd != NULL; pEd = pEd->next)
				{
					fprintf (fp, "E\n%s~\n%s~\n", pEd->keyword, fix_string (pEd->description));
				}
				for (door = 0; door < MAX_DIR; door++)
				{
					...
					{
						int locks = 0;
						if (IS_SET (pExit->rs_flags, EX_ISDOOR) && (!IS_SET (pExit->rs_flags, EX_CHANGELING)) && (!IS_SET (pExit->rs_flags, EX_BASHPROOF)) && (!IS_SET (pExit->rs_flags, EX_PASSPROOF)))
							locks = 1;
						if (IS_SET (pExit->rs_flags, EX_ISDOOR) && (IS_SET (pExit->rs_flags, EX_CHANGELING)) && (!IS_SET (pExit->rs_flags, EX_BASHPROOF)) && (!IS_SET (pExit->rs_flags, EX_PASSPROOF)))
							locks = 2;
						if (IS_SET (pExit->rs_flags, EX_ISDOOR) && (!IS_SET (pExit->rs_flags, EX_CHANGELING)) && (IS_SET (pExit->rs_flags, EX_BASHPROOF)) && (!IS_SET (pExit->rs_flags, EX_PASSPROOF)))
				...
				for (pRt = pRoomIndex->roomtext; pRt != NULL; pRt = pRt->next)
				{
...
void save_specials (FILE * fp, AREA_DATA * pArea)
{
	...
	for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
	{
		for (pMobIndex = mob_index_hash[iHash]; pMobIndex; pMobIndex = pMobIndex->next)
		{
			...
			{
				fprintf (fp, "M %d %s Load to: %s\n", pMobIndex->vnum, pMobIndex->spec_name, pMobIndex->short_descr);
			}
		}
	}
	...
	for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
	{
		for (pRoomIndex = room_index_hash[iHash]; pRoomIndex; pRoomIndex = pRoomIndex->next)
		{
			if (pRoomIndex->area == pArea)
			{
				for (door = 0; door < MAX_DIR; door++)
				{
					if ((pExit = pRoomIndex->exit[door]) && pExit->to_room && (IS_SET (pExit->rs_flags, EX_CLOSED) || IS_SET (pExit->rs_flags, EX_LOCKED)))
#if defined( VERBOSE )
						fprintf (fp, "D 0 %d %d %d The %s door of %s is %s\n", pRoomIndex->vnum, door, IS_SET (pExit->rs_flags, EX_LOCKED) ? 2 : 1, dir_name[door], pRoomIndex->name, IS_SET (pExit->rs_flags, EX_LOCKED) ? "closed and locked" : "closed");
#endif
#if !defined( VERBOSE )
					fprintf (fp, "D 0 %d %d %d\n", pRoomIndex->vnum, door, IS_SET (pExit->rs_flags, EX_LOCKED) ? 2 : 1);
#endif
				}
			}
		}
...
void save_mpresets (FILE *fp, AREA_DATA * pArea)
{
	MPROG_RESETS *temp;
	if(pArea->mprogs != NULL)
	{
		fprintf (fp, "#MOBPROGS\n");
		for(temp=pArea->mprogs;temp != NULL;temp = temp->next)
			fprintf(fp, "M %d %s\n", temp->vnum, temp->filename);
		fprintf(fp, "S\n\n");
	}
	return;
}
...
void save_resets (FILE * fp, AREA_DATA * pArea)
{
	...
	for (pReset = pArea->reset_first; pReset; pReset = pReset->next)
	{
		...
		case 'M':
			...
			fprintf (fp, "M 0 %d %d %d Load %s\n", pReset->arg1, pReset->arg2, pReset->arg3, pLastMob->short_descr);
			break;

		case 'O':
			pLastObj = get_obj_index (pReset->arg1);
			pRoom = get_room_index (pReset->arg3);
			fprintf (fp, "O 0 %d 0 %d %s loaded to %s\n", pReset->arg1, pReset->arg3, capitalize (pLastObj->short_descr), pRoom->name);
			break;

		case 'P':
			pLastObj = get_obj_index (pReset->arg1);
			fprintf (fp, "P 0 %d 0 %d %s put inside %s\n", pReset->arg1, pReset->arg3, capitalize (get_obj_index (pReset->arg1)->short_descr), pLastObj->short_descr);
			break;
		...
		case 'E':
			fprintf (fp, "E 0 %d 0 %d %s is loaded %s of %s\n", pReset->arg1, pReset->arg3, capitalize (get_obj_index (pReset->arg1)->short_descr), wear_loc_name (pReset->arg3), pLastMob ? pLastMob->short_descr : "!NO_MOB!");
			if (!pLastMob)
			{
				sprintf (buf, "Save_resets: !NO_MOB! in [%s]", pArea->filename);
				bug (buf, 0);
...
void do_asave (CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0')
	{
		...
		send_to_char ("  asave zone     - saves the zone youre in\n\r", ch);
...
void do_arealist (CHAR_DATA * ch, char *argument)
{
	...
	send_to_char( "\n\r", ch );
	sprintf (buf, "#n[#e%3s#n]#e %-32s #n(#e%-5s-%5s#n)#e   %10s %3s #n[#e%s#n]\n\r", "#eNum#n", "#eArea Name#n", "#elvnum#n", "#euvnum#n", "#eFilename#n", "#eSec#n", "#eBuilders#n");
	for (pArea = area_first; pArea != NULL; pArea = pArea->next)
	{
		sprintf (addbuf, "#p[#e%3d#p]#n #P%-35s#n #n(#e%-5d-%5d#n)#n\n\r      #p%13s#n #e%2d#n #p[#e%s#p]#n\n\r", 
