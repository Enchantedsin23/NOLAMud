...
char *exitflag_bit_name (int exit_flags)
{
	...
	if (exit_flags & EX_LOCKED)
		strcat(buf, " locked");
	if (exit_flags & EX_BASHPROOF)
		strcat(buf, " bashproof");
	if (exit_flags & EX_BASHED)
		strcat(buf, " bashed");
	if (exit_flags & EX_CHANGELING)
		strcat(buf, " changeling");
	if (exit_flags & EX_PASSPROOF)
		strcat(buf, " passproof");
	if (exit_flags & EX_WALL_WATER)
		strcat(buf, " waterwall");
	if (exit_flags & EX_WARD_GHOULS)
		strcat(buf, " ward_ghouls");
	if (exit_flags & EX_WARD_LUPINES)
		strcat(buf, " ward_lupines");
	if (exit_flags & EX_WARD_KINDRED)
		strcat(buf, " ward_kindred");
	if (exit_flags & EX_WARD_SPIRITS)
		strcat(buf, " ward_spirits");
	if (exit_flags & EX_WALL_FLAME)
		strcat(buf, " flamewall");
	if (exit_flags & EX_WALL_GRANITE)
		strcat(buf, " granitewall");
...
char *room_bit_name (int room_flags)
{
	...
	if (room_flags & ROOM_SAFE)
		strcat (buf, " safe");
	if (room_flags & ROOM_BATH)
		strcat (buf, " bath");
	if (room_flags & ROOM_BAR)
		strcat (buf, " bar");
	if (room_flags & ROOM_CBAR)
		strcat (buf, " cbar");
	if (room_flags & ROOM_CHARGEN)
		strcat (buf, " chargen");
	if (room_flags & ROOM_TOTAL_DARKNESS)
		strcat (buf, " total_darkness");
	if (room_flags & ROOM_CLOTHES)
		strcat (buf, " clothes");
	if (room_flags & ROOM_HEDGEGATE)
		strcat (buf, " hedgegate");
	if (room_flags & ROOM_STOREROOM)
		strcat (buf, " storeroom");
	if (room_flags & ROOM_ESCAPE)
		strcat (buf, " escape");
return (buf[0] != '\0') ? buf + 1 : "";
}
...
int room_name_bit (char *buf)
{
	...
	if (!str_cmp (buf, "cbar"))
		return ROOM_CBAR;
	if (!str_cmp (buf, "chargen"))
		return ROOM_CHARGEN;
	if (!str_cmp (buf, "clothes"))
		return ROOM_CLOTHES;
	if (!str_cmp (buf, "total_darkness"))
		return ROOM_TOTAL_DARKNESS;
	if (!str_cmp (buf, "hedgegate"))
		return ROOM_HEDGEGATE;
	if (!str_cmp (buf, "storeroom"))
		return ROOM_STOREROOM;
	if (!str_cmp (buf, "escape"))
		return ROOM_ESCAPE;
	return 0;
...
int sector_number (char *argument)
{
	...
	if (!str_cmp (argument, "city"))
		return SECT_CITY;
	if (!str_cmp (argument, "sewer"))
		return SECT_SEWER;
	if (!str_cmp (argument, "forest"))
		return SECT_FOREST;
	if (!str_cmp (argument, "hedge"))
		return SECT_HEDGE;
...
{
	if (path == PATH_REGO_VITAE)
		return "Rego Vitae";
	if (path == PATH_CREO_IGNEM)
		return "Creo Ignem";
	if (path == PATH_REGO_MOTUS)
		return "Rego Motus";
	if (path == PATH_REGO_TEMPESTAS)
		return "Rego Tempestas";
	if (path == PATH_REGO_AQUAM)
		return "Rego Aquam";
	if (path == PATH_REGO_ELEMENTUM)
		return "Rego Elementum";
	if (path == PATH_ALCHEMY)
		return "Alchemy";
	if (path == PATH_LEVINBOLT)
		return "LevinBolt";
	if (path == PATH_SOULS_FEAR)
	  return "Prey On Souls Fear";
	if (path == PATH_WARDING)
		return "Warding";
	if (path == PATH_REGO_VENALIS)
		return "Rego Venalis";
	if (path == PATH_NEFAS)
		return "Nefas";
	if (path == PATH_REGO_DOLOR)
		return "Rego Dolor";
	if (path == PATH_REGO_MANES)
		return "Rego Manes";
	if (path == PATH_WATER)
		return "Water";
	if (path == PATH_FIRE)
...
int item_name_type (char *name)
{
	...
	if (!str_cmp (name, "food"))
		return ITEM_FOOD;
	if (!str_cmp (name, "contract"))
		return ITEM_CONTRACT;
	if (!str_cmp (name, "money"))
		return ITEM_MONEY;
	if (!str_cmp (name, "condom"))
		return ITEM_CONDOM;
	...
	if (!str_cmp (name, "stake"))
		return ITEM_STAKE;
	if (!str_cmp (name, "scent"))
		return ITEM_SCENT;
	if (!str_cmp (name, "fruit"))
		return ITEM_FRUIT;
...
int get_liquid_type( char *arg )
{
    ...
    if (!str_cmp( arg, "stout"   		) ) return 4;
    if (!str_cmp( arg, "whiskey"		) ) return 5;
    if (!str_cmp( arg, "brandy"		    ) ) return 6;
    if (!str_cmp( arg, "rum"		    ) ) return 7;
    if (!str_cmp( arg, "gin"	        ) ) return 8;
    if (!str_cmp( arg, "liquor"     	) ) return 9;
...
char * get_type_liquid( int arg )
{
    ...
    case 6:  return "brandy";
    case 7:  return "rum";
    case 8:  return "gin";
    case 9:  return "liquor";
    case 10: return "milk";
    case 11: return "tea";
    case 12: return "coffee";
    case 13: return "blood";
    case 14: return "salt_water";
    case 15: return "cola";
    }
