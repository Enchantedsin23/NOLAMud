		     ...
		     OBJ_DATA *obj, void *vo, CHAR_DATA *rndm)
{
  ...
  *argpt = '\0';
  ...
  if ( !str_cmp( buf, "rand" ) )
    {
      return ( number_percent2() <= atoi(arg) );
    }
  ...
  if ( !str_cmp( buf, "poundsamt" ) )
    {
      switch ( arg[1] )  /* arg should be "$*" so just get the letter */
	{
	case 'i': lhsvl = mob->pounds;
                  rhsvl = atoi( val );
                  return mprog_veval( lhsvl, opr, rhsvl );
	case 'n': if ( actor )
	          {
	...
	case 'r': if ( rndm )
		    ...
		    return -1;
	default:
	  bug ( "Mob: %d bad argument to 'poundsamt'", mob->pIndexData->vnum ); 
	  return -1;
	}
    }
	...
	if ( ( list[0] == 'p' ) && ( list[1] == ' ' ) )
	  {
	    ...
	    {
		log_string("top", CHANNEL_LOG_NORMAL);
		log_string(dupl, CHANNEL_LOG_NORMAL);   
	...
	else
	  {
	    ...
	    {
		log_string("bottom", CHANNEL_LOG_NORMAL);
		log_string(dupl, CHANNEL_LOG_NORMAL);
		...
		if ( ( start == dupl || *(start-1) == ' ' )
			...
			|| end == NULL ) )
		  {
		    log_string("true speechprog", CHANNEL_LOG_NORMAL);
		    mprog_driver( mprg->comlist, mob, actor, obj, vo );
		    return;
		  }
		else
...
bool spec_cast_adept (CHAR_DATA * ch)
{
	...
	{
	case 0:
		act ("$n utters the word 'nahkalas'.", ch, NULL, NULL, TO_ROOM);
		return TRUE;

	case 1:
		act ("$n utters the word 'htilil'.", ch, NULL, NULL, TO_ROOM);
		return TRUE;

	case 2:
		act ("$n utters the word 'airolam'.", ch, NULL, NULL, TO_ROOM);
		return TRUE;

	case 3:
		act ("$n utters the word 'niyyahk'.", ch, NULL, NULL, TO_ROOM);
		return TRUE;

	case 4:
		act ("$n utters the words 'nriak'.", ch, NULL, NULL, TO_ROOM);
		return TRUE;

	case 5:
		act ("$n utters the words 'obbog'.", ch, NULL, NULL, TO_ROOM);
		return TRUE;
