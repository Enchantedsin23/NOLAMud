...
HELP_DATA *new_help(void) // Remove this whole thing if stuff gets weird 12/26/16
{
     HELP_DATA *NewHelp;
...
NULL))
	...
	{
//sprintf(buf, "DEBUG CHECK ON %s:\n ARG: %s\n PLAYER MIEN/DESC: %s/%s\n TARGET MIEN/DESC: %s/%s\n RESULT: %s\n", rch->name, arg, ch->pcdata->miendesc, ch->pcdata->roomdesc, rch->pcdata->miendesc, rch->pcdata->roomdesc, strcasestr(rch->pcdata->miendesc, arg));
//send_to_char(buf, ch);
		if (!IS_NPC (rch) && IS_EXTRA (rch, EXTRA_OSWITCH))
			continue;
		else if (!can_see (ch, rch))
			continue;
		else if ( IS_CHANGELING(ch) && IS_CHANGELING(rch) )
		{
			if ( strcasestr( rch->pcdata->miendesc, arg ) == NULL ) {
//send_to_char("MIEN CHECK FAILED", ch);
				continue;
			} else {
//send_to_char("MIEN CHECK PASSED", ch);
			}
		}
		// Returns NULL if arg isn't in roomdesc
		else if (!IS_NPC (rch) && strcasestr(rch->pcdata->roomdesc, arg) == NULL) // ???
		{
			continue;
		}
		if (++count == number)
