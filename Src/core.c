...
bool is_safe (CHAR_DATA * ch, CHAR_DATA * victim)
{
	...
	{
		send_to_char ("#eYou realize you're a Storyteller, right? Try the #cburn#e or #cregain#e commands.#n\n\r", ch);
		return FALSE;
	}
...
void make_corpse (CHAR_DATA * ch, CHAR_DATA * maker)
{
	...
	if (IS_NPC (ch))
	{
		...
		if (ch->pounds > 0)
		{
			obj_to_obj (create_money (ch->pounds), corpse);
			ch->pounds = 0;
		}
	}
	else
	{
		...
		/* Why should players keep their pounds? */
		if (ch->pounds > 0)
		{
			obj = create_money (ch->pounds);
