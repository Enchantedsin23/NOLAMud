...
sh_int			gsn_insert_page;
sh_int			gsn_remove_page;
...
/*	weather_info.mmhg	= 960;
	 ...
	 * Load pose/renown recommendations
	 */
	{
		fix_exits ();
		fBootDb = FALSE;
...
//		pMobIndex->affected_by2 = fread_number (fp);
		...
		pMobIndex->pounds = fread_number (fp);	/* Unused */
		...
		pMobIndex->abilities[PHYSICAL][TAL_BRAWL] = fread_number (fp);
		pMobIndex->abilities[PHYSICAL][TAL_STEALTH] = fread_number (fp);
		pMobIndex->abilities[SOCIAL][SKI_STREETWISE] = fread_number (fp);
...
void load_objects (FILE * fp)
{
	...
	for (;;)
	{
		...
		pObjIndex->inside_desc = NULL;
		pObjIndex->outside_desc = NULL;
...
*/
		for (;;)
		{
			...
			{
				pObjIndex->inside_desc = fread_string (fp);
				pObjIndex->outside_desc = fread_string (fp);
			}
		...
		for (;;)
		{
			...
			if (letter == 'D')
			{
				...
				case 1:
					...
					break;
				case 2:
					pexit->rs_flags = EX_ISDOOR | EX_CHANGELING;
					break;
				}
		...
		for (;;)
		{
			...
			if (letter == 'D')
			{
				...
				case 1:
					...
					break;
				case 2:
					pexit->exit_info = EX_ISDOOR | EX_CHANGELING;
					break;
				}
...
//      exit( 1 );
	...
	*obj = obj_zero;
	...
	if (pObjIndex->inside_desc != NULL)
	{
		obj->inside_desc = str_dup(pObjIndex->inside_desc);
		obj->outside_desc = str_dup(pObjIndex->outside_desc);
...
/* End finger elements */
		...
		free_string (ch->pcdata->gaurudesc);
		free_string (ch->pcdata->tempdesc);
		free_string (ch->pcdata->biography);
		free_string (ch->pcdata->urhandesc);
		free_string (ch->pcdata->daludesc);
		free_string (ch->pcdata->urshuldesc);
...
void do_areas (CHAR_DATA * ch, char *argument)
{
	...
	send_to_char( "#RAreas#r of #RInnocence Lost#r:#n\n\r", ch);
	sprintf(buf,"#w%-36s #e|#n %-30s#n\n\r", "Area Name", "Builders");
	for (pArea = area_first; pArea != NULL; pArea = pArea->next)
	{
		sprintf (addbuf, "#r[#R%-35s#r] #r(#R%-30s#r)#n\n\r", 
